...
#include <unistd.h>
#include <string.h>
#include "include/mtee/uree/system.h"
#include "include/mtee/uree/mem.h"
#include "include/mtee/tz_cross/ta_mem.h"
...
TZ_RESULT UREE_AllocSecurechunkmem(UREE_SESSION_HANDLE session,
    ...
    ret = _allocFunc(TZCMD_MEM_SECURECM_ALLOC, session, cm_handle, alignment, size, "UREE_AllocSecurechunkmem", NULL);

    return ret;
}

TZ_RESULT UREE_AllocSecurechunkmemWithTag(UREE_SESSION_HANDLE session,
    UREE_SECUREMEM_HANDLE *cm_handle, uint32_t alignment, uint32_t size, const char *tag) {
    TZ_RESULT ret;

    ret = _allocFunc(TZCMD_MEM_SECURECM_ALLOC_WITH_TAG, session, cm_handle, alignment, size, "UREE_AllocSecurechunkmem", tag);
